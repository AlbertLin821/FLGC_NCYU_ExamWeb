...
- 學生交卷後，系統**僅自動計分**選擇題與多選；問答题（essay）可留白，且不會在交卷時呼叫 AI。
- 教師於「成績後台」選定**班級與單一考卷**後，可按「集體 AI 批閱問答题」，對該班已交卷、問答题尚未計分之場次逐人送一次模型請求；成功後寫入各題分數與整卷雙語總評。
...
- **AI 評分**：問答题集體批閱或後續手動流程中，若模型輸出無法解析或雲端配額不足，可能寫入 `pending_review`（待人工複閱）；**交卷仍會成功**，成績後台會顯示「已評分（待複閱）」等狀態，與純「已評分」區分。
